...
class Token implements IHttpStatusCode
{
  ...
  /*
  * @param request
  * @return array(userId, tokenAlive)
  */
  public function DecodeToken($request)
  {
    try {
      $payload = \JWT::decode($request, $this->serverKey, array('HS256'));
      $payload = get_object_vars($payload);

      $start_ts = strtotime(date('m/d/Y H:i:s', $payload['c']));
      $end_ts = strtotime(date('m/d/Y H:i:s', $payload['l']));
      $user_ts = strtotime(date('m/d/Y H:i:s', time())); //now

      $token = \explode(".", $payload['t']);
      $uniqid = substr($token[2], 0, $token[0]);
      $userId = substr($token[2], $token[0]);

      return array(
        "userId" => $userId,
        "tokenAlive" => (bool) (($user_ts >= $start_ts) && ($user_ts <= $end_ts))
      );
    } catch (\Throwable $th) {
      //throw $th;
    }
